...
class Toy_CAE_SS:
	...
	def __init__(self, sess, xmlfilename):
		...
		self.model_name = ut.readSpecificTagXML(root, 'model_name')
		self.learning_rate = float(ut.readSpecificTagXML(root, 'learning_rate'))
		self.batch_size_labels = int(ut.readSpecificTagXML(root, 'batch_size_labeled'))
		self.batch_size_unlabels = int(ut.readSpecificTagXML(root, 'batch_size_unlabeled'))
		self.epochs = int(ut.readSpecificTagXML(root, 'num_epochs'))
		self.print_iter = int(ut.readSpecificTagXML(root, 'print_iter'))
		self.save_iter = int(ut.readSpecificTagXML(root, 'save_iter'))
		self.reduced_data = int(ut.readSpecificTagXML(root, 'reduced_data')) # number of data per label to use for training
		...
		self.dataAll = np.load('../data/ToyData/Data.npy')
		self.labelsAll = np.load('../data/ToyData/labels.npy')
		[self.orig_data, self.orig_labels, self.orig_masks, self.orig_data_val, self.orig_labels_val, self.orig_data_test, self.orig_labels_test] = ut.partitionToy_SS(self.dataAll, self.labelsAll, self.reduced_data)
		if not os.path.exists(self.log_dir):
			os.makedirs(self.log_dir)
		self.logFN = self.log_dir + '/logFile_' + str(self.bottleneck) + '_' + str(self.batch_size_labels) + '_' + str(self.cae_weight_init) + '.csv'
		self.TBLog = self.log_dir + '/' + str(self.bottleneck) + '_' + str(self.batch_size_labels) + '_' + str(self.cae_weight_init) + '/' + 'log4'
	...
	def build_model(self):
		...
		self.labels = tf.placeholder(tf.float32, [None, 4], name='orig_labels')
		self.phase_train = tf.placeholder(tf.bool, name='phase')
		self.mask = tf.placeholder(tf.float32, [None, 4], name="orig_masks") 
		...
		self.temp = self.mask*tf.nn.sigmoid_cross_entropy_with_logits(logits=self.out_labels, labels=self.labels)
		# self.temp = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.out_labels, labels=self.labels)
		self.cross_entropy = tf.reduce_mean(self.temp)
		...
		t_vars = tf.trainable_variables()
		with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
			self.optim_burnin = tf.train.AdamOptimizer(0.0001).minimize(self.cross_entropy, var_list=t_vars)
			self.optim = tf.train.AdamOptimizer(self.learning_rate).minimize(self.loss, var_list=t_vars)
			# self.optim_cae = tf.train.AdamOptimizer(self.learning_rate).minimize(self.cae_weight*self.cae_loss, var_list=t_vars)
		...
		with tf.name_scope('performance-evaluation'):
			...
			self.tb_acc3 = tf.placeholder(tf.float32, shape=None, name='acc-summary3')
			self.tb_ce3 = tf.placeholder(tf.float32, shape=None, name='cross-entropy-summary3')
			self.tb_cae3 = tf.placeholder(tf.float32, shape=None, name='cae-loss-summary3')
			self.tb_tl3 = tf.placeholder(tf.float32, shape=None, name='total-loss-summary3')
			self.tb_acc_summ = tf.summary.scalar('accuracy-train-lab', self.tb_acc)
			self.tb_ce_summ = tf.summary.scalar('cross-entropy-train-lab', self.tb_ce)
			self.tb_cae_summ = tf.summary.scalar('cae-loss-train-lab', self.tb_cae)
			self.tb_tl_summ = tf.summary.scalar('total-loss-train-lab', self.tb_tl)
			...
			self.tb_acc_summ3 = tf.summary.scalar('accuracy-train-unlab', self.tb_acc3)
			self.tb_ce_summ3 = tf.summary.scalar('cross-entropy-train-unlab', self.tb_ce3)
			self.tb_cae_summ3 = tf.summary.scalar('cae-loss-train-unlab', self.tb_cae3)
			self.tb_tl_summ3 = tf.summary.scalar('total-loss-train-unlab', self.tb_tl3)

		# with tf.name_scope('feature-stats'):		
	...
	def train_model(self):
		...
		with open(self.logFN, 'w') as csvfile:
			spamwriter = csv.writer(csvfile, delimiter=',')
			for epoch in range(start_epoch, self.epochs):
				[shufLab_data, shufLab_labels, shufUnLab_data, shufUnLab_labels] = ut.training_shuffle_data_SS(self.orig_data, self.orig_labels, self.orig_masks)
				caeWT = self.cae_weight_init + (epoch/self.epochs)*(self.cae_weight_final - self.cae_weight_init)
				if epoch < self.burn_in_epochs:
					self.num_batches = int(shufLab_data.shape[0] / self.batch_size_labels)
					for idx in range(start_batch_id, self.num_batches):
						[batch_data_train, batch_labels_train] = ut.get_batch_data(shufLab_data, shufLab_labels, idx, self.batch_size_labels)
						[batch_data_val, batch_labels_val] = ut.get_batch_data(self.orig_data_val, self.orig_labels_val, idx, self.batch_size_labels)
						batch_data_masks = np.ones([batch_data_train.shape[0], 4])
						self.optim_burnin.run(session=self.sess, feed_dict={self.input_data: batch_data_train, self.labels: batch_labels_train, self.mask: batch_data_masks, self.phase_train:True, self.cae_weight : caeWT})
						counter += 1
	
				else:
					print("with CAE")
					nb_lab = int(shufLab_data.shape[0] / self.batch_size_labels)
					nb_unlab = int(shufUnLab_data.shape[0] / self.batch_size_unlabels)					
					self.num_batches = np.max([nb_lab, nb_unlab])
					wch = np.argmax([nb_lab, nb_unlab])
					# print(self.num_batches, wch, nb_lab, self.batch_size_labels) 
					for idx in range(start_batch_id, self.num_batches):
						if wch == 0:
							# means more labels then unlabeled
							[bt_lab_data, bt_lab_lab] = ut.get_batch_data(shufLab_data, shufLab_labels, idx, self.batch_size_labels)
							idxalt = int(np.mod(idx*self.batch_size_unlabels, shufUnLab_data.shape[0])/self.batch_size_unlabels)
							[bt_unlab_data, bt_unlab_lab] = ut.get_batch_data(shufUnLab_data, shufUnLab_labels, idxalt, self.batch_size_unlabels)
						else:
							# means more unlabels then labels
							# print("Here Goes")
							[bt_unlab_data, bt_unlab_lab] = ut.get_batch_data(shufUnLab_data, shufUnLab_labels, idx, self.batch_size_unlabels)
							idxalt = int(np.mod(idx*self.batch_size_labels, shufLab_labels.shape[0])/self.batch_size_labels)
							# print(idx, idxalt)
							[bt_lab_data, bt_lab_lab] = ut.get_batch_data(shufLab_data, shufLab_labels, idxalt, self.batch_size_labels)
						valNum = int(self.orig_data_val.shape[0] / 50)
						idxval = np.mod(idx*50, valNum)	
						# print(idxval)
						[batch_data_val, batch_labels_val] = ut.get_batch_data(self.orig_data_val, self.orig_labels_val, idxval, 50)

						batch_val_masks = np.ones([batch_data_val.shape[0], 4])
						batch_data_train = np.append(bt_lab_data, bt_unlab_data, 0)
						batch_labels_train = np.append(bt_lab_lab, bt_unlab_lab, 0)
						batch_data_masks = np.append(np.ones([bt_lab_data.shape[0], 4]), np.zeros([bt_unlab_data.shape[0], 4]), 0)
						self.optim.run(session=self.sess, feed_dict={self.input_data: batch_data_train, self.labels: batch_labels_train, self.mask: batch_data_masks, self.phase_train:True, self.cae_weight : caeWT})
						counter += 1

				[acc_lab, caeerr_lab, mtLs_lab, Ls_lab] = self.all_model_loss(shufLab_data, shufLab_labels)
				[acc_unlab, caeerr_unlab, mtLs_unlab, Ls_unlab] = self.all_model_loss(shufUnLab_data, shufUnLab_labels)
				[acc_val, caeerr_val, mtLs_val, Ls_val] = self.all_model_loss(self.orig_data_val, self.orig_labels_val)	
				summ_train = self.sess.run(self.performance_summaries, feed_dict={self.tb_acc:acc_lab, self.tb_ce:mtLs_lab, self.tb_cae:caeerr_lab, self.tb_tl:Ls_lab,self.tb_acc2:acc_val, self.tb_ce2:mtLs_val, self.tb_cae2:caeerr_val, self.tb_tl2:Ls_val, self.tb_acc3:acc_unlab, self.tb_ce3:mtLs_unlab, self.tb_cae3:caeerr_unlab, self.tb_tl3:Ls_unlab})
				self.writer.add_summary(summ_train, epoch)
				l = [epoch, acc_lab, acc_unlab, acc_val, caeerr_lab, caeerr_unlab, caeerr_val, mtLs_lab, mtLs_unlab, mtLs_val, Ls_lab, Ls_unlab, Ls_val]
				spamwriter.writerow(l)
				print("-----------------------------")
				print("Training-Labeled: Epoch: [%2d], accuracy: %.8f, Match-Loss: %.8f, CAE-Err: %.8f, Tot-Loss: %.8f" %(epoch, acc_lab, mtLs_lab, caeerr_lab, Ls_lab))
				print("Training-Unlabeled: Epoch: [%2d], accuracy: %.8f, Match-Loss: %.8f, CAE-Err: %.8f, Tot-Loss: %.8f" %(epoch, acc_unlab, mtLs_unlab, caeerr_unlab, Ls_unlab))
		...
		savedir = '../experiments/ToySS/' + str(self.bottleneck) + '_' + str(self.reduced_data) + '/'
	...
	def test_model(self):
		...
		allaccuracyCAE = 0
		cae_err = 0
		print(self.orig_data_test.shape[0], numE)
		for i in range(numE):
			batchTestData = self.orig_data_test[i*loadBT:(i+1)*loadBT, :]
			batchTestLabels = self.orig_labels_test[i*loadBT:(i+1)*loadBT, :]
	...
	def VisualizeModel(self):
		...
		allaccuracyCAE = 0
		cae_err = 0
		outputLabels = np.zeros([self.labelsAll.shape[0], self.labelsAll.shape[1]])
	...
	def getScatter(self, labels, Val, trueLabels, ttl1, ttl2):
		...
		if self.bottleneck == 1:
			...
			for i in range(4):
				...
				f = f[0]
				# print(f.shape)
				plt.scatter(Val[f, 0], np.ones([len(f), 1]))
			plt.title(ttl1)
			plt.subplot(122)
			for i in range(4):
				f = np.where(trueLabels == i)
	...
	def validation_model(self, step):
		'''
		Implements the testing routine.
		Also computes the CAE loss for reconstruction accuracy
	...
	def all_model_loss(self, data, labels):
		...
		for i in range(numE):
			...
			accuracy_test, orig_feature, cae_feature, match_loss, loss = self.sess.run([self.accuracy, self.out_feature, self.cae_out, self.cross_entropy, self.loss],
				feed_dict={self.input_data: batchTestData, self.labels: batchTestLabels, self.phase_train:False, self.mask:masks, self.cae_weight:self.cae_weight_init})
			allaccuracy += accuracy_test
			allmatchloss += match_loss
			allloss += loss
			temp = np.sqrt(np.sum((orig_feature - cae_feature)**2))
	...
	def save_separate(self, checkpoint_dir, model_dir, model_name, bt, rd, step):
		model_name = model_name + '_' + str(bt) + '_' + str(rd) + '_' + str(self.cae_weight_init)
		checkpoint_dir = os.path.join(checkpoint_dir, model_dir, model_name)
		if not os.path.exists(checkpoint_dir):
			os.makedirs(checkpoint_dir)
		self.saver.save(self.sess, os.path.join(checkpoint_dir, model_name + '.model'), global_step=step)
	...
	def load_separate_step(self, checkpoint_dir, model_dir, model_name, bt, rd, step):
		model_name = model_name + '_' + str(bt) + '_' + str(rd) + '_' + str(self.cae_weight_init)
		checkpoint_dir = os.path.join(checkpoint_dir, model_dir, model_name)
		# ckpt = tf.train.get_checkpoint_state(checkpoint_dir)
		# ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
		ckpt_name = model_name + '.model-' + str(step)
		print('Loading Checkpoint ...')
		print(ckpt_name)
		self.saver.restore(self.sess, os.path.join(checkpoint_dir, ckpt_name))
