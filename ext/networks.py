...
class MNIST_Base_Classification:
	...
	def classifier(self, x, phase_train, is_training=True, reuse=False):
		...
		with tf.variable_scope('classifier', reuse=reuse):
			...
			with tf.name_scope('fc2'):
				W_fc2 = ut.weight_variable([1024, 10], 'fc2_wt')
				b_fc2 = ut.bias_variable([10], 'fc2_bias')
	...
	def build_model(self):
		...
		# self.cross_entropy = tf.reduce_mean(tf.nn.l2_loss((self.labels - self.out_labels)))
	...
	def train_model(self):
		...
		with open(self.logFN, 'w') as csvfile:
			...
			for epoch in range(start_epoch, self.epochs):
				...
				for idx in range(start_batch_id, self.num_batches):
					...
					# 	# training accuracy
					# 	# validation accuracy
					# 	accuracy_val, out_val, match_val = self.sess.run([self.accuracy, self.out_labels, self.cross_entropy],
					# 		feed_dict={self.input_data: batch_data_val, self.labels: batch_labels_val, self.phase_train:False})	
					# 	l = [epoch, idx, accuracy_train, accuracy_val, match_train, match_val]
					# 	spamwriter.writerow(l)
					# 	print("TRAINING : Epoch: [%2d] [%4d/%4d] time: %4.4f, accuracy: %.8f, Match-Loss: %.8f" %(epoch, idx, self.num_batches, time.time() - start_time, accuracy_train, match_train))
					# 	print("VALIDATION : Epoch: [%2d] [%4d/%4d] time: %4.4f, accuracy: %.8f, Match-Loss: %.8f," %(epoch, idx, self.num_batches, time.time() - start_time, accuracy_val, match_val))

				# total_loss_val = total_loss_val / self.num_batches
				total_loss_train = total_loss_train / self.num_batches
				total_accuracy_train = total_accuracy_train / self.num_batches
				print("TRAINING : Epoch: [%2d] time: %4.4f, accuracy: %.8f, Match-Loss: %.8f" %(epoch, time.time() - start_time, total_accuracy_train, total_loss_train))
				...
				all_train_loss.append(total_loss_train)
				...
				if np.mod(epoch, self.save_iter) == 0:
					...
					# print('Ho Ho HO')
					# print(out_val[1, :])
					# print(self.orig_labels_val[1, :])
					self.save(self.checkpoint_dir, self.model_dir, self.model_name, counter)

		# X_features = np.zeros([7*7*128, self.orig_data.shape[0]])
		# for i in range(self.orig_data.shape[0]):
		# 	batchTestData = self.orig_data[i, :].reshape(1, 784)
		# 	batchTestLabels = self.orig_labels[i, :].reshape(1, 10)
		...
		# np.save('../experiments/MNISTCase1/Features/features_train.npy', X_features)
		np.save('../experiments/MNISTCase1/all_train_accuracy.npy', all_train_acc)
		# np.save('../experiments/MNISTCase1/all_val_accuracy.npy', all_val_acc)
		np.save('../experiments/MNISTCase1/all_train_loss.npy', all_train_loss)
		# np.save('../experiments/MNISTCase1/all_val_loss.npy', all_val_loss)
	...
	def PCA_test_model(self):
		...
		print(numE)   
		allaccuracy = 0
		X_features = np.zeros([7*7*128, numE])
		Y_features = np.zeros([7*7*128, numE])
		for i in range(numE):
			batchTestData = self.orig_data_test[i, :].reshape(1, 784)
			batchTestLabels = self.orig_labels_test[i, :].reshape(1, 10)
			...
			temp = np.array(temp)
			X_features[:, i]= temp[0,0,:]
		np.save('../experiments/MNISTCase1/Features/features_test.npy', X_features)
...
class MNIST_l2_Classification:
	...
	def feature_extractor(self, x, phase_train, is_training=True, reuse=False):
		...
		with tf.variable_scope("feature_extractor", reuse=reuse):
			x_in = tf.reshape(x, [-1, 28, 28, 1])
			[conv1_1, regVal] = ut.conv_layer_reg(x_in, 1, 64, 'conv1_1', 3, phase_train)
			# conv1_2 = ut.conv_layer(conv1_1, 64, 128, 'conv1_2', 3, phase_train)
			with tf.name_scope('pool1'):
				h_pool1 = ut.max_pool_2x2_2d(conv1_1)
			# conv2_1 = ut.conv_layer(h_pool1, 128, 256, 'conv2_1', 3, phase_train)
			[conv2_2, temp] = ut.conv_layer_reg(h_pool1, 64, 128, 'conv2_2', 3, phase_train)
			regVal = regVal + temp
			with tf.name_scope('pool2'):
				h_pool2 = ut.max_pool_2x2_2d(conv2_2)
				h_pool2_flat = tf.reshape(h_pool2, [-1, 7*7*128])
			return [h_pool2_flat, regVal]
	...
	def classifier(self, x, phase_train, regVal, is_training=True, reuse=False):
		...
		with tf.variable_scope('classifier', reuse=reuse):
			...
			with tf.name_scope('fc2'):
				...
				b_fc2 = ut.bias_variable([10], 'fc2_bias')
				out = tf.matmul(h_fc1, W_fc2) + b_fc2
				regVal = regVal + tf.nn.l2_loss(W_fc2)
			return [out, regVal]
	...
	def build_model(self):
		...
		# define the cross entropy loss
		# self.cross_entropy = tf.reduce_mean(-tf.reduce_sum(self.labels * tf.log(self.out_labels), reduction_indices=[1]))
		self.cross_entropy = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.out_labels, labels=self.labels)) + self.reg_wt*self.regVal
		# self.cross_entropy = tf.reduce_mean(tf.nn.l2_loss((self.labels - self.out_labels)))
		correct_pred = tf.equal(tf.argmax(self.labels, 1), tf.argmax(self.out_labels, 1))
		self.accuracy = tf.reduce_mean(tf.cast(correct_pred, tf.float32), name='accuracy')
...
class MNIST_Case2_Classification:
	...
	def train_model(self):
		...
		total_accuracy_train = total_accuracy_train/self.epochs
		total_accuracy_val = total_accuracy_val/self.epochs
...
class MNIST_Case3_Classification:
	...
	def __init__(self, sess, xmlfilename):
		...
		self.l1_weight = float(ut.readSpecificTagXML(root, 'l1_weight')) 
		# read and process the data sequence
		mnist = input_data.read_data_sets("../data/MNIST/MNIST_data/", one_hot=True)
		[self.orig_data, self.orig_labels] = ut.reduceMNIST(mnist, self.reduced_data)
		print(self.orig_data.shape)
	...
	def classifier(self, x, phase_train, is_training=True, reuse=False):
		...
		with tf.variable_scope('classifier', reuse=reuse):
			...
			with tf.name_scope('fc4'):
				W_fc4 = ut.weight_variable([1024, 10], 'fc4_wt')
				b_fc4 = ut.bias_variable([10], 'fc4_bias')
				out = tf.nn.softmax(tf.matmul(h_fc3, W_fc4) + b_fc4)
			return [out, normout]
	...
	def build_model(self):
		...
		self.out_feature = self.feature_extractor(self.input_data, self.phase_train, is_training=True, reuse=False)
		[self.out_labels, self.regul] = self.classifier(self.out_feature, self.phase_train, is_training=True, reuse=False)
		...
		t_vars = tf.trainable_variables()
		with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
			self.optim = tf.train.AdamOptimizer(self.learning_rate).minimize(self.cross_entropy + self.l1_weight*self.regul, var_list=t_vars)
	...
	def train_model(self):
		...
		for epoch in range(start_epoch, self.epochs):
			...
			if np.mod(counter, self.save_iter) == 0:
				# save the check point
				self.save(self.checkpoint_dir, self.model_dir, self.model_name, counter)
...
class MNIST_Case4_Classification:
	...
	def train_model(self):
		...
		with open(self.logFN, 'w') as csvfile:
			...
			for epoch in range(start_epoch, self.epochs):
				...
				# total_loss_val = total_loss_val / self.num_batches
				# total_loss_train = total_loss_train / self.num_batches
				# match_loss_train = match_loss_train / self.num_batches
				# cae_loss_train = cae_loss_train / self.num_batches
				# total_accuracy_train = total_accuracy_train / self.num_batches
				# total_accuracy_val = total_accuracy_val / self.num_batches
				# [total_accuracy_val, match_loss_val, cae_loss_val, total_loss_val] = self.sess.run([self.accuracy, self.cross_entropy, self.cae_loss, self.loss],
				# 			feed_dict={self.input_data: self.orig_data_val, self.labels: self.orig_labels_val, self.phase_train:False, self.cae_weight : caeWT})	
				# [total_accuracy_train, match_loss_train, cae_loss_train, total_loss_train] = self.sess.run([self.accuracy, self.cross_entropy, self.cae_loss, self.loss],
				# 			feed_dict={self.input_data: self.orig_data, self.labels: self.orig_labels, self.phase_train:False, self.cae_weight : caeWT})	

				# all_train_loss.append(total_loss_train)
				# match_train_loss.append(match_loss_train)
				# cae_train_loss.append(cae_loss_train)
				# all_train_acc.append(total_accuracy_train)
				# all_val_acc.append(total_accuracy_val)
				# all_val_loss.append(total_loss_val)
				# cae_val_loss.append(cae_loss_val)
				# match_val_loss.append(match_loss_val)

				if np.mod(epoch, self.save_iter) == 0:
					# save the check point
					self.save_separate(self.checkpoint_dir, self.model_dir, self.model_name, self.bottleneck, self.reduced_data, counter)

		savedir = '../experiments/MNISTCase4/' + str(self.bottleneck) + '_' + str(self.reduced_data) + '/'
		if not os.path.exists(savedir):
			os.makedirs(savedir)
		# np.save(savedir + 'all_train_accuracy.npy', all_train_acc)
		# np.save(savedir + 'all_val_accuracy.npy', all_val_acc)
		# np.save(savedir + 'all_train_loss.npy', all_train_loss)
		# np.save(savedir + 'match_train_loss.npy', match_train_loss)
		# np.save(savedir + 'cae_train_loss.npy', cae_train_loss)
		# np.save(savedir + 'all_val_loss.npy', all_val_loss)
		# np.save(savedir + 'match_val_loss.npy', match_val_loss)
		# np.save(savedir + 'cae_val_loss.npy', cae_val_loss)
	...
	def test_model_singlemagic(self):
		...
		loadBT = 10
		numE = int(self.orig_data_test.shape[0]/loadBT)
		allscalar = np.zeros([numE*loadBT, 1])
		alllabel = np.zeros([numE*loadBT, 1])
		cae_err = 0
		for i in range(numE):
			batchTestData = self.orig_data_test[i*loadBT:(i+1)*loadBT, :]
			batchTestLabels = self.orig_labels_test[i*loadBT:(i+1)*loadBT, :]

			btnk = self.sess.run([self.bottleneck_value],
				feed_dict={self.input_data: batchTestData, self.labels: batchTestLabels, self.phase_train:False})
			# print(btnk[0][0])
			allscalar[i*loadBT:(i+1)*loadBT, :] = btnk[0]
			alllabel[i*loadBT:(i+1)*loadBT, 0] = np.argmax(batchTestLabels, 1)

		np.save('bottleneck1values.npy', allscalar)
		np.save('allassociatedlabels.npy', alllabel)
...
class MNIST_Case5_Classification:
	...
	def train_model(self):
				...
				for idx in range(start_batch_id, self.num_batches):
					...
					if np.mod(counter, self.print_iter) == 0:
						...
						accuracy_val, match_val, cae_val, featureNV, tot_val = self.sess.run([self.accuracy, self.cross_entropy, self.cae_loss, self.l1Loss, self.loss],
							feed_dict={self.input_data: batch_data_val, self.labels: batch_labels_val, self.phase_train:False, self.cae_weight : caeWT, self.emptyPH:batch_emptyPH_val})	
						l = [epoch, idx, accuracy_train, accuracy_val, match_train, match_val, cae_train, cae_val, featureNT, featureNV]
						spamwriter.writerow(l)
		...
		savedir = '../experiments/MNISTCase5/' + str(self.bottleneck) + '_' + str(self.reduced_data) + '/'
		if not os.path.exists(savedir):
			os.makedirs(savedir)
		# np.save(savedir + 'all_train_accuracy.npy', all_train_acc)
		# np.save(savedir + 'all_val_accuracy.npy', all_val_acc)
